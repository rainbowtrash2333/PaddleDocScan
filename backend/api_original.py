...
def ai_analysis() -> Union[Dict[str, Any], Tuple[Dict[str, Any], int]]:
    """
    AI内容分析接口
    ...
    try:
        # 获取请求数据
        if not request.json:
            return APIResponse.error("请求数据格式错误", "INVALID_JSON")
        ...
        # 验证输入
        if not content:
            return APIResponse.error("分析内容不能为空", "EMPTY_CONTENT")
        
        if len(content) > 50000:  # 限制内容长度
            return APIResponse.error("内容长度超过限制（最多50000字符）", "CONTENT_TOO_LONG")
        
        # 执行AI分析
        try:
            analysis_result = ai_analysis_service.analyze_content(content, analysis_type)
            
            # 清理内存
            gc.collect()
            
            logger.info(f"AI分析完成，内容长度: {len(content)}")
            return APIResponse.success(analysis_result, "AI分析完成")
            
        except AIAnalysisError as e:
            logger.error(f"AI分析失败: {e}")
            return APIResponse.error(f"AI分析失败: {str(e)}", "AI_ANALYSIS_ERROR", 500)
        
    except Exception as e:
        logger.error(f"AI分析接口异常: {e}")
        return APIResponse.error("AI分析服务暂时不可用", "AI_SERVICE_ERROR", 500)
